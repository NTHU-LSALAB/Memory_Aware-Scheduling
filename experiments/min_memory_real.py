...
if not skip:
    ...
    with open('./experiments/results/real_result.json', 'w') as f:
        json.dump({
            "ticks": xticks_flatlist,
            "data": min_data_3d
        }, f, indent=4)
else:
    with open('./experiments/results/real_result.json', 'r') as f:
        result = json.load(f)
        xticks_flatlist = result['ticks']
        min_data_3d = result['data']
...
ax.legend(list(map(lambda method: method[1], method_list)), fontsize=12)
