...
app = App()
app.read_input('samples/sample.1.json')
...
X = np.arange(3)
fig, ax = plt.subplots()
ax.bar(X + 0.0, data, color='#1F77B4', width=0.25)
ax.set_xticks(X, labels=depths)
# ax_exec.legend(labels=['heft-lookup', 'heft-delay', 'heft'])
ax.set_title("Impact of depth")
ax.set_xlabel(f'Depth')
ax.set_ylabel('Degree of Parallelism')
fig.savefig(f'experiments/results/depth.png')
